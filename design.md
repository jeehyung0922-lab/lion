...
| 카드 채우기 | `#111111` @ **25~30%** |
| 카드/입력 라벨 | `#888888` |
| 일반 테두리 | `white / 10%` (단, **홈은 `white / 20%`** — 아래 참고) |
| 슬라이더 채움·썸 | `#D9D9D9` |
| 세그먼트 토글 활성 | `#555555` @ **20%** (+ 유리) |

> ⚠️ **홈(메인페이지)만 예외로 테두리를 `white/20`으로 올림**(2026-08-18). 온보딩은 `white/10` 그대로 유지. 홈의 박스들이 GNB·카드 v2 화면들과 나란히 눈에 띄는데 `white/10`은 밝은 배경 위에서 거의 안 보여서, GNB(카드 v2 계열과 통일)에 맞춰 홈의 루틴 표 셀·체크인 카드·무월 게이지·"일정 조율하기" 버튼·근거 드롭다운도 `white/20`으로 올렸다.
